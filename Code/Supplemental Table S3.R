...
for (outcome in c("2y", "5y", "not5y", "all")){
  if (outcome=="2y"){
    data_summary <- cohort |>
      dplyr::filter(outcome_2y==1)
  } else if (outcome=="5y"){
    data_summary <- cohort |>
      dplyr::filter(outcome_5y==1)
  } else if (outcome=="not5y"){
    data_summary <- cohort |>
      dplyr::filter(outcome_5y==0 | outcome_5y==2)
  } else if (outcome=="all"){
    data_summary <- cohort
  }
  
  table_ovr_gtsummary <- data_summary |>
    dplyr::ungroup() |>
  ...
  assign(paste0("table_ovr_gtsummary_", outcome), table_ovr_gtsummary)
}

# merge three tables
merged_gt <- gtsummary::tbl_merge(list(table_ovr_gtsummary_2y, 
                          table_ovr_gtsummary_5y, 
                          table_ovr_gtsummary_not5y,
                          table_ovr_gtsummary_all))
# Convert to gt table
table_ovr_gt <- gtsummary::as_gt(merged_gt) |>
  gt::tab_header(title = "Table 1. Baseline characteristics stratified by outcome")

# Save the table to a Word document
gt::gtsave(table_ovr_gt, "~/Results/Table 1. Baseline Characteristics stratified.docx")
